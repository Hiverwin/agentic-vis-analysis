...
test('workspace store description helpers normalize widget descriptions with stable defaults', () => {
  const description = makeWidgetDescription({
    widgetId: 'scatter_a',
    recognizedKinds: ['scatter'],
  })

  assert.equal(description.widgetId, 'scatter_a')
  assert.deepEqual(description.recognizedKinds, ['scatter'])
  assert.equal(description.role, 'primary')
  assert.equal(description.humanInteraction.mode, 'none')
  assert.equal(description.humanInteraction.supportsDirectManipulation, false)
})

test('workspace store description helpers normalize nested transport and planning payloads', () => {
  ...
  assert.equal('optionalTools' in description.transportHints, false)
...
test('workspace store description helpers expose stable defaults', () => {
  ...
  assert.equal('optionalTools' in transportHints, false)
...
test('readWorkspaceDescriptionFromStore derives a workspace description from plain manual-assembly store fields', () => {
    ...
    actions: {
      [actionName]: {
        ...
        effects: {
          ...
          targetStatePaths: ['shared.selections'],
        },
      },
    },
    perceptionQueries: {
  ...
  assert.equal('analyticalPlacement' in description.actions[0], false)
  assert.equal('sharedAnalyticalSurface' in description.actions[0], false)
  assert.equal(description.perceptionQueries[0]?.name, perceptionName)
})

test('readWorkspaceDescriptionFromStore merges executor-exposed workspace actions into store action surface', () => {
  const widgetRef = 'wl://demo/workspace/main/widget/scatter_a'
  const store = {
    appId: 'demo',
    workspaceId: 'main',
  ...
  const actionExecutor = new ActionExecutor({ store })

  const description = readWorkspaceDescriptionFromStore(store, { actionExecutor })
  const actionNames = description.actions.map((action) => action.name)

  assert.equal(actionNames.includes('widget.updateSelection'), true)
  assert.equal(actionNames.includes('coordination.applyLink'), true)
})
...
test('readWorkspaceDescriptionFromStore normalizes kind-only coordination links from plain manual-assembly facades', () => {
  const sourceWidgetRef = 'wl://demo/workspace/main/widget/scatter_a'
  const firstTargetWidgetRef = 'wl://demo/workspace/main/widget/bar_b'
  const secondTargetWidgetRef = 'wl://demo/workspace/main/widget/detail_c'
  const firstLinkRef = 'wl://demo/workspace/main/link/scatter_filters_bar'
  const secondLinkRef = 'wl://demo/workspace/main/link/scatter_filters_detail'
  ...
  const description = readWorkspaceDescriptionFromStore({
    ...
    descriptions: {
      ...
      [secondTargetWidgetRef]: { ref: secondTargetWidgetRef, kind: 'detail', title: 'Detail C' },
    },
    links: {
      [firstLinkRef]: {
        ref: firstLinkRef,
  ...
  assert.deepEqual(
    description.links.map((link) => ({ kind: link.kind, hasPrimitive: Object.hasOwn(link, 'primitive') })),
    [
      { kind: 'filter', hasPrimitive: false },
      { kind: 'filter', hasPrimitive: false },
    ],
  )
  assert.equal(description.benchmarkSupport, undefined)
