...
test('WidgetRegistry normalizes primitive-only workspace links into WidgetLink contract shape', () => {
  const registry = new WidgetRegistry()
  ...
  assert.equal(describedLink?.primitive, 'filter')
  assert.equal(resolvedLink?.kind, 'filter')
  assert.equal(resolvedLink?.primitive, 'filter')
})
