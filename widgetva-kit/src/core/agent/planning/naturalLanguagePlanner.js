...
function buildAgentMessages({ objective, observe, knowledge = null, history = null, plannerContext = null, responseRequirements = null }) {
  ...
  const systemPrompt = [
    ...
    'If operation.kind is action or perception, include operation.target.widgetRef. When the selected tool supplies target.widgetRef, copy it exactly; otherwise copy one full ref string from observe.state.widgets[].ref.',
...
function operationMatchesTargetFamily(operation = {}, observe = {}, knowledge = null) {
  ...
  if (directTools) {
    return directTools.some((tool) => (
      ...
      && (!tool?.target?.widgetRef || tool.target.widgetRef === operation?.target?.widgetRef)
    ))
  }
...
function buildSafeFallbackOperation(observe = {}, knowledge = null) {
  ...
  const perceptionNames = directTools
    ? directTools
      .filter((tool) => tool?.kind === 'perception' && (!tool?.target?.widgetRef || tool.target.widgetRef === focusedWidgetRef))
      .map((tool) => tool?.name)
      .filter((name, index, names) => typeof name === 'string' && name.length > 0 && names.indexOf(name) === index)
    : [
