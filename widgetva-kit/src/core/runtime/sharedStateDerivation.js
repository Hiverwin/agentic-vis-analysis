import { extractPredicatesFromTransforms } from './materializers/transformHelpers.js'
...
export function deriveHighlightStateFromSelection(primarySelection = null, widgets = []) {
  ...
  const highlightedKeys = uniqueValues(
    predicates
      .filter((predicate) => predicate?.op === 'equals' && predicate?.value != null)
...
export function deriveGlobalFiltersFromSelection(primarySelection = null, { rangeDomains = {} } = {}) {
  ...
  for (const predicate of predicates) {
    ...
    if (predicate.field === 'origin' && predicate.op === 'equals' && typeof predicate.value === 'string') {
      globalFilters.origin = predicate.value
      continue
    }
    if (predicate.field === 'year' && predicate.op === 'equals' && Number.isFinite(predicate.value)) {
      globalFilters.year = Number(predicate.value)
      continue
    }
    if (predicate.field === 'cylinders' && predicate.op === 'equals' && Number.isFinite(predicate.value)) {
