...
function inferFieldFromSpec(rawSpec, channel) {
  ...
  if (Array.isArray(rawSpec?.layer) && rawSpec.layer.length > 0) {
    for (const layer of rawSpec.layer) {
      if (layer?.encoding?.[channel]?.field) return layer.encoding[channel].field
    }
  }
  return null
}
...
function buildLineAnomalyResult({ rows, rawSpec, threshold = 2, yField = null, xField = null }) {
  ...
  if (!resolvedYField) {
    throw new Error('perception.detectAnomalies requires a yField or a line spec with an encoded y field.')
  }
