...
test('InteractionTraceRecorder.recordDataQuery stores the DataQueryCall directly in trace records', () => {
...
test('InteractionTraceRecorder records failed perception and data queries as trace records', () => {
...
test('InteractionTraceRecorder.describeRecorder exposes latest normalized trace metadata', () => {
  const recorder = new InteractionTraceRecorder({
    store: {
      interactionTrace: [
        {
...
test('InteractionTraceRecorder falls back to plain manual-assembly snapshot and trace facades', () => {
  const store = {
    stateId: 'main:s2',
    version: 0,
    stateSnapshots: [
  ...
  const recorder = new InteractionTraceRecorder({ store })
