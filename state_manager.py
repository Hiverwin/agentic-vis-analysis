...
def tool_output(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if "vega_spec" in call_kwargs and isinstance(call_kwargs["vega_spec"], dict):
            call_kwargs["state"] = _normalize_input_state(call_kwargs.pop("vega_spec"))
        elif "state" in call_kwargs and isinstance(call_kwargs["state"], dict):
            call_kwargs["state"] = _normalize_input_state(call_kwargs["state"])
        elif call_args and isinstance(call_args[0], dict):
            call_args[0] = _normalize_input_state(call_args[0])
...
def _normalize_input_state(payload: Dict[str, Any]) -> Dict[str, Any]:
    state, data = StateManager.split(payload) if "data" in payload else (payload, DataStore.get())
    if data is not None:
        DataStore.set(data)
    return state
