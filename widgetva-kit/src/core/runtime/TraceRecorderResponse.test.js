...
test('TraceRecorder response summary exposes latest response provenance counters', () => {
  ...
  const summary = recorder.describeResponseRecorder()
...
test('TraceRecorder response summary scopes counters to the current workspace', () => {
  const recorder = new TraceRecorder({
    store: {
      workspaceId: 'main',
      responseHistory: [
...
test('TraceRecorder response recording normalizes records through the shared response contract', () => {
  const appended = []
  const recorder = new TraceRecorder({
    store: {
      workspaceId: 'main',
...
test('TraceRecorder response recording preserves usage metadata for downstream token-cost metrics', () => {
  const appended = []
  const recorder = new TraceRecorder({
    store: {
      workspaceId: 'main',
