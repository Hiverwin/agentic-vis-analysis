import { $ as initLinearScaleMapper, $a as mathMin, $t as ChartView, Aa as error, At as registerPreprocessor, Cr as XY, Ct as registerCoordinateSystem, Dn as ComponentModel, Dt as registerMap, Et as registerLoading, Ft as registerVisual, It as setCanvasCreator, Lt as version, Mt as registerTheme, N as makeLabelFormatter, Nt as registerTransform, Ot as registerPostInit, Pa as warn, Pt as registerUpdateLifecycle, Qa as mathMax, Qt as throttle, Si as clonePath, Sr as WH, St as registerAction, Tt as registerLayout, V as OrdinalScale, X as decorateScaleMapper, Z as enableScaleMapperFreeze, _i as Polygon, _t as getCoordinateSystemDimensions, a as parseGeoJSON, ao as parsePercent, ar as getScaleBreakHelper, bt as getMap, ci as updateProps, dt as connect, fr as Model, ft as dataTool, gi as Polyline, gt as dispose, ha as makeInner, hi as Line, ht as disconnect, ii as isElementRemoved, jt as registerProcessor, kt as registerPostUpdate, l as use, lr as registerLocale, lt as DataDiffer, mr as animateLabelValue, mt as disConnect, ni as getOldStyle, nn as SERIES_UNIVERSAL_TRANSITION_PROP, ot as SeriesData, pa as isValidBoundsForExtent, pt as dependencies, qi as getECData, ri as initProps, rn as SeriesModel, sr as registerScaleBreakHelperImpl, t as Axis, ti as getAnimationConfig, tn as ComponentView, to as mathRound, ut as PRIORITY, vt as getInstanceByDom, wt as registerCustomSeries, xt as init, ya as normalizeToArray, yi as Sector, yr as labelInner, yt as getInstanceById } from "./Axis-BPvfGPOG.js";
import { An as defaults, Bn as isArray, Bt as Point, D as Displayable, E as fromPoints, En as clone$1, Et as cubicSubdivide, F as Group, Fn as filter, In as find, Jt as rotate, Kt as matrix_exports, Mn as each, Nt as BoundingRect, On as createHashMap, P as zrender_exports, Pn as extend, Qn as keys, Rn as indexOf, Sn as vector_exports, Un as isFunction, Ut as create, V as Transformable, Yn as isString, _r as setPlatformAPI, cr as retrieve2, er as map, f as Rect, fr as trim, m as subPixelOptimizeLine, mn as lerp, on as applyTransform, pr as util_exports$1, qt as mul, r as brushSingle, ut as color_exports, vr as env, w as PathProxy, wn as assert, x as normalizeRadian, y as Path } from "./graphic-Byb0a---.js";
import { a as util_exports, c as time_exports, i as extendSeriesModel, l as number_exports, n as extendComponentModel, o as format_exports, r as extendComponentView, s as graphic_exports, t as extendChartView, u as helper_exports } from "./api-Dp9wrgV5.js";
import { C as install$1, D as updateLabelLinePoints, E as setLabelLineStyle, S as install$2, T as getLabelLineStatesModels, _ as install$6, a as install$18, b as install$3, c as install$15, d as install$12, f as install$11, g as install$7, h as install$8, i as install$19, l as install$14, m as install$9, n as install$21, o as install$17, p as install$10, r as install$20, s as install$16, t as install$22, u as install$13, v as install$5, w as install, x as installScatterJitter, y as install$4 } from "./install-DYS3FkpF.js";
import { Bt as restoreIgnore, It as hideOverlap, Lt as labelIntersect, Rt as labelLayoutApplyTranslation, St as registerAxisBreakHelperImpl, Vt as shiftLayoutOnXY, bt as registerAction$1, nt as registerLegacyGridContainLabelImpl, s as install$24, vt as AXIS_BREAK_EXPAND_ACTION_TYPE, y as install$23, zt as newLabelLayoutWithGeometry } from "./customGraphicKeyframeAnimation-BVOvVGol.js";
import { n as install$26, t as install$25 } from "./installCanvasRenderer-Cr4CZk19.js";
import { C as install$31, D as install$27, E as install$28, O as install$35, S as install$32, T as install$29, _ as install$38, a as install$46, b as install$34, c as install$43, d as install$42, g as install$39, h as install$40, i as install$49, l as install$45, m as install$41, n as install$51, o as install$48, r as install$50, s as install$47, t as install$52, u as install$44, v as install$37, w as install$30, x as install$33, y as install$36 } from "./install-BdzV595k.js";
...
function pathToBezierCurves(path) {
	...
	function addLine(x0, y0, x1, y1) {
		if (!(aroundEqual(x0, x1) && aroundEqual(y0, y1))) currentSubpath.push(x0, y0, x1, y1, x1, y1);
	}
...
function splitPolygonByLine(points, lineA, lineB) {
	...
	if (intersections.length < 2) return [{ points }, { points }];
	...
	for (var i = splitPt0.idx + 1; i <= splitPt1.idx; i++) addToPoly(newPolyA, points[i].slice());
	addToPoly(newPolyA, splitPt1Arr);
	addToPoly(newPolyA, splitPt0Arr);
	for (var i = splitPt1.idx + 1; i <= splitPt0.idx + len; i++) addToPoly(newPolyB, points[i % len].slice());
...
function clone(path, count) {
	var paths = [];
	for (var i = 0; i < count; i++) paths.push(clonePath(path));
	return paths;
}
...
function polygonConvert(points) {
	var out = [];
	for (var i = 0; i < points.length;) out.push([points[i++], points[i++]]);
	return out;
}
function split(path, count) {
	...
	switch (path.type) {
		...
		default:
			...
			if (polygonCount === 0) binaryDivideRecursive(binaryDividePolygon, { points: polygons[0] }, count, outShapes);
			else if (polygonCount === count) for (var i = 0; i < polygonCount; i++) outShapes.push({ points: polygons[i] });
			else {
				var totalArea_1 = 0;
	...
	if (!OutShapeCtor) return clone(path, count);
	var out = [];
...
function combineMorph(fromList, toPath, animationOpts) {
	...
	function addFromPath(fromList) {
		for (var i = 0; i < fromList.length; i++) {
			...
			else if (from instanceof Path) fromPathList.push(from);
		}
	}
	...
	if (individualDelay) {
		...
		toPath.animateTo({ __morphT: 1 }, defaults({
			during: function(p) {
				...
				restoreToPath();
				for (var i = 0; i < fromList.length; i++) restoreMethod(fromList[i], "updateTransform");
				oldDone && oldDone();
			}
		}, animationOpts));
	}
...
function separateMorph(fromPath, toPathList, animationOpts) {
	...
	if (isCombineMorphing(fromPath)) {
		...
		if (fromLen < toLen) {
			var k = 0;
			for (var i = fromLen; i < toLen; i++) fromPathList.push(clonePath(fromPathList[k++ % fromLen]));
		}
	...
	var individualDelay = animationOpts.individualDelay;
	for (var i = 0; i < toLen; i++) {
		var indivdualAnimationOpts = individualDelay ? defaults({ delay: (animationOpts.delay || 0) + individualDelay(i, toLen, fromPathList[i], toPathList[i]) }, animationOpts) : animationOpts;
...
function applyMorphAnimation(from, to, divideShape, seriesModel, dataIndex, animateOtherProps) {
	...
	var animationCfg = extend({ setToFinal: true }, updateAnimationCfg);
	var many;
	var one;
	if (isMultiple(from)) {
		many = from;
	...
	function morphOneBatch(batch, fromIsMany, animateIndex, animateCount, forceManyOne) {
		...
		if (batchMany.length === 1 && !forceManyOne) {
			...
			if (isCombineMorphing(batchFrom)) morphOneBatch({
				...
				var individualAnimationCfg = animationDelay ? defaults({ delay: animationDelay(animateIndex, animateCount) }, animationCfg) : animationCfg;
			...
			var _a = fromIsMany ? combineMorph(batchMany, batchOne, separateAnimationCfg) : separateMorph(batchOne, batchMany, separateAnimationCfg), fromIndividuals = _a.fromIndividuals, toIndividuals = _a.toIndividuals;
			var count = fromIndividuals.length;
			for (var k = 0; k < count; k++) {
				var individualAnimationCfg = animationDelay ? defaults({ delay: animationDelay(k, count) }, animationCfg) : animationCfg;
...
function getPathList(elements) {
	...
	if (isArray(elements)) {
		var pathList_1 = [];
		for (var i = 0; i < elements.length; i++) pathList_1.push(getPathList(elements[i]));
		return pathList_1;
	}
	var pathList = [];
	elements.traverse(function(el) {
		if (el instanceof Path && !el.disableMorphing && !el.invisible && !el.ignore) pathList.push(el);
	});
	return pathList;
}
//#endregion
...
function fadeInElement(newEl, newSeries, newIndex) {
	newEl.traverse(function(el) {
		if (el instanceof Path) initProps(el, { style: { opacity: 0 } }, newSeries, {
			dataIndex: newIndex,
			isFrom: true
		});
	});
...
function transitionBetween(oldList, newList, api) {
	...
	*/
	...
	if (!useId) for (var i = 0; i < newDiffItems.length; i++) {
		var newItem = newDiffItems[i];
	...
	function updateOneToOne(newIndex, oldIndex) {
		...
		if (newEl) {
			...
			} else fadeInElement(newEl, newSeries, newIndex);
		}
	}
	new DataDiffer(oldDiffItems, newDiffItems, createKeyGetter(true, useId), createKeyGetter(false, useId), null, "multiple").update(updateOneToOne).updateManyToOne(function(newIndex, oldIndices) {
		...
		if (newEl) {
			...
			} else fadeInElement(newEl, newSeries, newItem.dataIndex);
		}
		...
		if (newElsList.length) {
			...
			} else each(newElsList, function(newEl) {
	...
	if (hasMorphAnimation) each(newList, function(_a) {
		var seriesModel = _a.data.hostModel;
		...
		if (view && seriesModel.isAnimationEnabled() && animationCfg && animationCfg.duration > 0) view.group.traverse(function(el) {
			if (el instanceof Path && !el.animators.length) el.animateFrom({ style: { opacity: 0 } }, animationCfg);
		});
	});
}
...
function findTransitionSeriesBatches(globalStore, params) {
	...
	each(globalStore.oldSeries, function(series, idx) {
		...
		if (isArray(transitionKey)) each(transitionKey, function(key) {
...
function querySeries(series, finder) {
	for (var i = 0; i < series.length; i++) if (finder.seriesIndex != null && finder.seriesIndex === series[i].seriesIndex || finder.seriesId != null && finder.seriesId === series[i].id) return i;
}
...
function installUniversalTransition(registers) {
	registers.registerUpdateLifecycle("series:beforeupdate", function(ecMOdel, api, params) {
		each(normalizeToArray(params.seriesTransition), function(transOpt) {
			each(normalizeToArray(transOpt.to), function(finder) {
				...
				for (var i = 0; i < series.length; i++) if (finder.seriesIndex != null && finder.seriesIndex === series[i].seriesIndex || finder.seriesId != null && finder.seriesId === series[i].id) series[i][SERIES_UNIVERSAL_TRANSITION_PROP] = true;
			});
		});
	});
	registers.registerUpdateLifecycle("series:transition", function(ecModel, api, params) {
		...
		if (globalStore.oldSeries && params.updatedSeries && params.optionChanged) {
			...
			each(params.updatedSeries, function(series) {
				if (series["__universalTransitionEnabled"]) series[SERIES_UNIVERSAL_TRANSITION_PROP] = false;
			});
		}
...
* AUTO-GENERATED FILE. DO NOT MODIFY.
*/
var BreakScaleMapperImpl = function() {
	...
	*/
	...
	BreakScaleMapperImpl.decoratedMethods = {
		...
		getExtentUnsafe: function(kind, depth) {
			return depth == null || depth === 2 ? this._outOfBrk.getExtentUnsafe(kind, null) : this._linear.getExtentUnsafe(kind, null);
		},
		...
		setExtent2: function(kind, start, end) {
			if (isValidBoundsForExtent(start, end)) {
				if (kind === 0) updateAxisBreakGapReal(this, [start, end]);
		...
		*/
		transformOut: function(elapsedVal, opt) {
			if (opt && opt.depth === 2) return elapsedVal;
...
function updateAxisBreakGapReal(brkMapper, scaleExtent) {
	...
	each(brkMapper.breaks, function(brk) {
		var gapParsed = brk.gapParsed;
		if (gapParsed.type === "tpPrct") brk.gapReal = gapPrctSum !== 0 ? mathMax(prctBrksGapRealSum, 0) * gapParsed.val / gapPrctSum : 0;
...
function parseAxisBreakOption(breakOptionList, scale, opt) {
	...
	return { breaks: filter(parsedBreaks, function(brk) {
		return !!brk;
	}) };
}
...
function getTicksBreakOutwardTransform(scale, tick, outermostBreaks, lookup) {
	...
	var parsedBreak = {
		vmin: scale.transformOut(brk.vmin, opt),
		vmax: scale.transformOut(brk.vmax, opt),
		breakOption: brk.breakOption,
		gapParsed: clone$1(originalBrkItem.gapParsed),
		gapReal: brk.gapReal
	};
	return {
...
function removeUnusedCacheVisual(visualList) {
	for (var i = visualList.length - 1; i >= 0; i--) if (visualList[i].shouldRemove) visualList.splice(i, 1);
}
function rectCoordBuildBreakAxis(axisGroup, axisView, axisModel, coordSysRect, api) {
	...
	var group = new Group({ ignoreModelZ: true });
	...
	for (var i = 0; i < breakPairs.length; i++) _loop_1(i);
	axisGroup.add(group);
...
function buildAxisBreakLine(axisModel, group, transformGroup, pathBaseProp) {
	...
	for (var idx = 0; idx < brkLayoutList.length; idx++) {
		...
		if (ySegMin <= brkTirmmedMin) addSeg(ySegMin, brkTirmmedMin, lastLayout, layout);
...
function adjustBreakLabelPair(axisInverse, axisRotation, layoutPair) {
	...
	if (isParallelToAxis(0) || isParallelToAxis(1)) {
		...
		k = (qval - (uvalMax < 0 ? uvalMax : uvalMin > 0 ? uvalMin : 0)) / mtvSt.x;
	}
...
function updateModelAxisBreak(model, payload) {
	...
	each(payload.breaks, function(inputBrk) {
		...
		breakOption.isExpanded = actionType === "expandAxisBreak" ? true : actionType === "collapseAxisBreak" ? false : actionType === "toggleAxisBreak" ? !breakOption.isExpanded : breakOption.isExpanded;
		result.breaks.push({
