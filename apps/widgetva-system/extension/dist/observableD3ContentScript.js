(function() {
	...
	function createOfficialPageAgentBridgeMessageHandler(root) {
		return (event) => {
	...
	//#endregion
	//#region extension/src/shared/officialPageDockBridge.js
	var WIDGETVA_DOCK_BRIDGE_SOURCE_CONTENT = "widgetva-dock-content";
	var WIDGETVA_DOCK_BRIDGE_REQUEST = "widgetva:dock-request";
	//#endregion
	//#region extension/src/content/dock/officialPageDockClient.js
	var DEFAULT_DOCK_TIMEOUT_MS = 3e4;
	var DEFAULT_AGENT_TIMEOUT_MS = 3e5;
	...
	function createOfficialPageDockClient(root = window) {
		return {
			request(method, params = {}, { timeoutMs = method === "runSession" ? DEFAULT_AGENT_TIMEOUT_MS : DEFAULT_DOCK_TIMEOUT_MS, onProgress = null } = {}) {
				...
				return new Promise((resolve, reject) => {
					...
					const handleMessage = (event) => {
						const message = event?.data;
						if (event.source !== root || !message || message.source !== "widgetva-dock-page" || message.type !== "widgetva:dock-response" || message.id !== requestId) return;
	...
	function requestAgentRuntime(method, params = null) {
		return new Promise((resolve, reject) => {
			const runtime = globalThis.chrome?.runtime;
			if (!runtime || typeof runtime.sendMessage !== "function") {
				reject(makeError({ message: "WidgetVA extension runtime is unavailable." }));
				return;
			}
			runtime.sendMessage({
				type: WIDGETVA_AGENT_BRIDGE_RUNTIME,
				...
				if (runtime.lastError) {
					reject(makeError({ message: runtime.lastError.message || "WidgetVA extension runtime failed." }));
					return;
				}
				if (response?.ok !== true) {
	...
	//#endregion
	//#region extension/src/content/dock/officialPageDockModel.js
	function clone(value) {
		return value == null ? value : JSON.parse(JSON.stringify(value));
	}
	function compactNames(items = []) {
		return Array.from(new Set((Array.isArray(items) ? items : []).map((item) => typeof item === "string" ? item : item?.name).filter((name) => typeof name === "string" && name.length > 0)));
	}
	...
	function labelLink(link = null) {
		...
		return `${link.sourceAction || link.sourceStep || link.sourceStateRef || link.sourceWidgetId || link.sourceRef || link.source || "source"} -> ${link.targetAction || link.targetStep || link.targetStateRef || link.targetWidgetId || link.targetRef || link.target || "target"}`;
	}
	function compactLinks(links = []) {
		...
		for (const link of Array.isArray(links) ? links : []) {
			...
			const label = kind === "sharesSelection" && (!source || source === "source") && (!target || target === "target") ? "Shared selection across views" : labelLink(link);
			const key = [
				kind || "",
				label,
				source || "",
	...
	function extractDockWorkspaceSummary(snapshot = {}) {
		...
		const perceptions = compactNames(workspace.perceptionQueries || workspace.perceptions || workspace.queries);
	...
	function readObservedState(turn = {}) {
		...
		return observedState && typeof observedState === "object" && !Array.isArray(observedState) ? clone(observedState) : null;
	}
	...
	function readSnapshotRecoverableState(snapshot = {}) {
		...
		if (recoverableState && typeof recoverableState === "object" && !Array.isArray(recoverableState)) return clone(recoverableState);
		return null;
	}
	function readStateId(turn = {}, fallbackStateId = null) {
		const act = readAct(turn);
		return act.stateId || act?.result?.actionResult?.stateId || act?.result?.stateId || turn?.result?.actionResult?.stateId || turn?.result?.stateId || turn?.verify?.stateId || turn?.verify?.afterStateId || turn?.verification?.stateId || fallbackStateId || null;
	}
	...
	function readRecoverableState(turn = {}, stateId = null) {
		const act = readAct(turn);
		return clone(act.recoverableState || act?.result?.actionResult?.recoverableState || act?.result?.recoverableState || act?.result?.afterView || act?.result?.finalSnapshot || turn?.result?.actionResult?.recoverableState || turn?.result?.recoverableState || turn?.result?.afterView || turn?.result?.finalSnapshot || turn?.verify?.afterView || turn?.verify?.finalSnapshot || turn?.verification?.afterView || turn?.verification?.finalSnapshot || (stateId ? { stateId } : null));
	}
	...
	function summarizeState({ stateId, updatedRefs, verifySummary } = {}) {
		if (stateId && updatedRefs.length > 0) return `State ${stateId} updated ${updatedRefs.join(", ")}.`;
	...
	function buildDockTraceSteps({ session = {}, snapshot = {} } = {}) {
...
`;
	...
	function summarizeTraceParams(params = {}) {
		...
		return Object.entries(params).filter(([, value]) => value != null && value !== "").slice(0, 3).map(([key, value]) => {
	...
	function formatTracePopover(step = {}) {
		return [`Action: ${step.operation?.name || "None"}`, summarizeTraceParams(step.operation?.params || {})].filter((line) => typeof line === "string" && line.trim().length > 0).join("\n\n");
	}
	...
	function installWidgetVADock({ routeLabel = "Official page", provider = "unknown", root = window, ensurePageScript = null, openOnInstall = false } = {}) {
		if (document.getElementById(DOCK_HOST_ID)) return installedDockController;
  ...
  `;
		...
		function addMessage(role, text, { label = role, stepId = null, running = false } = {}) {
			state.messages.push({
				role,
				text,
				label,
		...
		function clearRunningMessages() {
			for (const message of state.messages) if (message.running) message.running = false;
		}
		...
		function renderHeader() {
			...
			elements.collapse.setAttribute("aria-label", state.collapsed ? "Expand WidgetVA Dock" : "Collapse WidgetVA Dock");
		}
		function renderSummary() {
			const summary = readSummary();
			elements.widget.textContent = summary.widget.kind ? `${summary.widget.kind} · ${summary.widget.label}` : "None";
			...
			const tags = [
				...
				},
				{
					label: "Links",
					title: "Links",
					body: summary.links.length > 0 ? summary.links.map((link) => {
						const suffix = [link.kind ? `(${link.kind})` : "", link.count > 1 ? `x${link.count}` : ""].filter(Boolean).join(" ");
		...
		function renderMessages() {
			...
			for (const message of state.messages) {
				...
				const step = message.stepId ? state.traceSteps.find((entry) => entry.id === message.stepId) || null : null;
				if (step) {
				...
				if (message.running) {
					...
					row.append(createElement("span", "spinner"), createElement("span", "activity-dots", message.text || ""));
		...
		function renderTrace() {
			...
			(state.traceSteps[0]?.id === "state_0" ? state.traceSteps : [{
				id: "state_0_display",
				label: "State 0",
				status: "success",
				operation: {
		...
		function showPopover(anchor, title, body) {
			...
			const left = Math.min(Math.max(12, rect.left - dockRect.left), Math.max(12, dockRect.width - 332));
			const top = Math.min(rect.bottom - dockRect.top + 8, Math.max(12, dockRect.height - 180));
		...
		async function bindPage() {
			...
			renderHeader();
			try {
				if (typeof ensurePageScript === "function") await ensurePageScript();
				state.statusText = "Binding";
				renderHeader();
				const snapshot = await client.request("bind", { forceReattach: true });
				state.snapshot = snapshot;
				state.bound = snapshot?.status === "ready";
				if (!state.bound) throw new Error(`WidgetVA binding did not reach ready. Status: ${snapshot?.status || "idle"}.`);
				state.status = state.bound ? "ready" : "idle";
				state.statusText = state.bound ? "Bound" : snapshot?.status || "Idle";
				if (state.bound) addMessage("system", "Page bound to WidgetVA runtime.", { label: "Bind" });
			} catch (error) {
				state.status = "failed";
		...
		function applySessionResult(result, durationMs) {
			...
			state.traceSteps = buildDockTraceSteps({
				session,
				snapshot
			});
			for (const step of state.traceSteps) if (step.reply && !hasMessageForStep(step.id)) addMessage("assistant", step.reply, {
		...
		function applySessionProgress(payload = {}) {
			...
			if (snapshot) state.snapshot = snapshot;
			const turns = Array.isArray(progress.turns) ? progress.turns : progress.turn ? [progress.turn] : [];
			...
			if (latestStep?.id) state.activeTraceStepId = latestStep.id;
			if (latestStep?.reply && !hasMessageForStep(latestStep.id)) addMessage("assistant", latestStep.reply, {
		...
		async function runObjective(objective, { recordUser = true } = {}) {
			...
			try {
				...
				if (next) runObjective(next, { recordUser: false });
			}
		}
		...
		elements.header.addEventListener("pointermove", (event) => {
			...
			if (Math.abs(deltaX) > 4 || Math.abs(deltaY) > 4) suppressHeaderClick = true;
		...
		function installTraceNodeDrag(node, step) {
			...
			node.addEventListener("pointermove", (event) => {
				...
				if (dragged) elements.traceScroll.scrollLeft = startScroll - delta;
			});
