...
async def lifespan(app: FastAPI):
    """启动时校验配置并打印，替代已弃用的 on_event('startup')。"""
...
# 存储每个 session 的动态建议（后台生成后写入，前端轮询读取）
...
def _clean_result_for_sse(result: Dict) -> Dict:
    # 先清理 iterations/explorations 里的大字段，避免 _strip_images 递归时内存爆炸
...
async def interrupt_session(session_id: str, body: InterruptBody):
    ...
    # fire-and-forget 持久化，不等待，确保接口立即响应（线程池满时不阻塞）
    loop = asyncio.get_running_loop()
...
async def query_stream(session_id: str, body: QueryBody):
    ...
    if session_id not in session_mgr.sessions:
        ...
        if restored_id != session_id:
            ...
            session_mgr.sessions[session_id] = session_mgr.sessions.pop(restored_id)
            session_mgr.sessions[session_id]["session_id"] = session_id
        # 恢复 DB 中的 spec_history / conversation_history / provenance_graph
        restored_session = session_mgr.get_session(session_id)
    ...
    async def generate():
        ...
        try:
            ...
            # fire-and-forget：不阻塞 SSE 流，持久化在后台异步执行
            ...
            # 先发一个轻量的 run.finishing 事件，让前端立即停止转圈
            # 避免 run.finished 大包序列化/传输延迟导致前端一直转
            yield {"data": _pack_event(AppEvents.RUN_FINISHING, {})}

            # Autonomous 硬协议：run.finished 前等待建议生成完成，再一次性下发。
            # 若超时/失败，则单次回退为 fallback（不再二次覆盖）。
...
if __name__ == "__main__":
    import uvicorn
    # 使用导入字符串才能正确启用 reload，且保证进程持续监听不退出
