...
} from '../../core/agent/planning/workspacePlanningShapes.js'
import {
  getInteractionTraceEventFamily,
  getInteractionTraceQueryName,
  getInteractionTraceQuerySurface,
} from '../../core/runtime/shapes/interactionTraceShapes.js'
import {
  makeBranchSummary,
  ...
  makeTraceGraphEdge,
  makeTraceGraphNode,
} from '../../core/runtime/shapes/historyShapes.js'
...
import {
  ...
  SHARED_STATE_REF,
  TASK_CONTEXT_REF,
} from '../../core/runtime/StateManager.js'

const WORKSPACE_TRANSPORT_TOOL_NAMES = [
  'workspace_describe',
  'view_read',
  'action_run',
  'perception_query',
...
function makeWorkspaceSpecSummary(summary = {}) {
  return {
...
export function readWidgetRefFromStateRef(stateRef) {
  if (typeof stateRef !== 'string' || stateRef.length === 0) return null
  const marker = '/widget/'
  const index = stateRef.lastIndexOf(marker)
  if (index < 0) return null
  const prefix = stateRef.slice(0, index + marker.length)
  const rest = stateRef.slice(index + marker.length)
  const widgetId = rest.split('/')[0]
  return widgetId ? `${prefix}${widgetId}` : null
}

export function readStateByRef(state = {}, stateRef = null) {
  if (typeof stateRef !== 'string' || stateRef.length === 0) return null
  const widgetRef = readWidgetRefFromStateRef(stateRef)
  if (!widgetRef) return state?.widgets?.[stateRef] || null
  const widgetState = state?.widgets?.[widgetRef] || null
  if (!widgetState) return null

  if (stateRef === widgetRef) return widgetState

  const suffix = stateRef.slice(widgetRef.length).replace(/^\/+/, '')
  const [kind, key] = suffix.split('/')

  if (kind === 'selection') {
    return widgetState.selections?.[stateRef] || widgetState.selections?.[key] || null
  }

  if (kind === 'transform') {
    return (Array.isArray(widgetState.transforms) ? widgetState.transforms : []).find((transform) => (
      transform?.ref === stateRef || transform?.id === key || transform?.params?.id === key
    )) || null
  }

  if (kind === 'view') {
    if (!key) return widgetState.view || null
    if (key === 'zoom') {
      return {
        ...(widgetState.view?.zoom || {}),
        ...(widgetState.view?.xDomain ? { xDomain: widgetState.view.xDomain } : {}),
        ...(widgetState.view?.yDomain ? { yDomain: widgetState.view.yDomain } : {}),
      }
    }
    return widgetState.view?.[key] ?? null
  }

  if (kind === 'encoding' || kind === 'encodings') {
    if (!key) return widgetState.encodings || null
    return widgetState.encodings?.[key] ?? null
  }
...
function actionDescriptorKey(descriptor) {
  return [
    descriptor?.name || '',
    descriptor?.targetRef || '',
  ].join('::')
}

function mergeActionDescriptorLists(primaryActions = [], supplementalActions = []) {
  const merged = []
  const seen = new Set()
  for (const descriptor of [...primaryActions, ...supplementalActions]) {
    if (!descriptor?.name) continue
    const key = actionDescriptorKey(descriptor)
...
export function listStoreLinks(store) {
  if (typeof store?.listLinks === 'function') {
    return (store.listLinks() || []).map((link) => stripWidgetLinkCompatibilityFields(makeWidgetLink(link)))
  }
  return valuesFromRecord(store?.links).map((link) => stripWidgetLinkCompatibilityFields(makeWidgetLink(link)))
}
...
export function listStoreActions(store, actionExecutor = null) {
  if (typeof store?.listActions === 'function') {
    ...
    if (Array.isArray(listedActions) && listedActions.length > 0) {
      const executorActions = typeof actionExecutor?.list === 'function' ? actionExecutor.list() : []
      return mergeActionDescriptorLists(listedActions, executorActions)
    }
  }
...
export function listStorePerceptionQueries(store, perceptionExecutor = null) {
  ...
  if (typeof perceptionExecutor?.list === 'function') {
    const listedQueries = perceptionExecutor.list()
    if (Array.isArray(listedQueries) && listedQueries.length > 0) {
      return listedQueries
    }
  }
...
export function readWorkspaceDescriptionFromStore(store, {
  ...
  if (typeof store?.readDescription === 'function') {
    ...
    const describedActions = Array.isArray(description?.actions) ? description.actions : []
    const executorActions = typeof actionExecutor?.list === 'function' ? actionExecutor.list() : []
    return makeWorkspaceDescription({
      ...description,
      actions: mergeActionDescriptorLists(describedActions, executorActions),
    })
  }
  const widgets = listStoreWidgets(store)
  ...
  return makeWorkspaceDescription({
    ...
    perceptionQueries: listStorePerceptionQueries(store, perceptionExecutor),
  })
}
