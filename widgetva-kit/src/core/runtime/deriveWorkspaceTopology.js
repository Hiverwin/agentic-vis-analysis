...
import { makeWorkspaceTopologySummary, normalizeWidgetLink } from '../protocol/widgetLinks.js'
...
export function isTopologyLink(link) {
  ...
  return TOPOLOGY_LINK_KINDS.has(normalizedLink?.kind) || TOPOLOGY_LINK_KINDS.has(normalizedLink?.primitive)
}
...
export function deriveWorkspaceTopology({ widgets = [], links = [] } = {}) {
  ...
  const linkEntries = (Array.isArray(links) ? links : []).map((link) => normalizeWidgetLink(link)).filter(isTopologyLink)
