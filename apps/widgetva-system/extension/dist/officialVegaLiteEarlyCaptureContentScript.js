(function() {
	...
	function installEarlyVegaEmbedCapture(root = globalThis.window) {
		...
		function wrapCapturedFunction(state, fn, source, rawKey, wrappedKey) {
			...
			if (state[rawKey] === fn && state[wrappedKey]) return state[wrappedKey];
		...
		function installCapturedProperty(host, state, propertyName, source, rawKey, wrappedKey) {
			let currentValue = typeof host[propertyName] === "function" ? wrapCapturedFunction(state, host[propertyName], source, rawKey, wrappedKey) : propertyName in host ? host[propertyName] : null;
			try {
				Object.defineProperty(host, propertyName, {
					...
					set(nextValue) {
						currentValue = typeof nextValue === "function" ? wrapCapturedFunction(state, nextValue, source, rawKey, wrappedKey) : nextValue;
					}
				});
			} catch {
		...
		if (!root || typeof root !== "object") throw new Error("installEarlyVegaEmbedCapture requires a browser-like root object.");
		if (root.__widgetvaVegaEmbedCapture) return root.__widgetvaVegaEmbedCapture;
